...
Recalibration notes (from analysis of full 100K dataset):
  - 610 candidates trigger the confirmed honeypot signature:
      skill_exceeds_career + skill_inflation + inverted_salary (all three simultaneously)
    These are confirmed artificial profiles (fictional companies, contradictory summaries,
    impossible skill durations). The challenge states ~80 as a rough estimate.
  - Use --strict to apply tighter thresholds that target closer to ~80 (score >= 0.55 only).
  - Default mode (score >= 0.30 or flags >= 3) catches all confirmed anomalies (610).
  - 38,700 SUSPICIOUS candidates have 1-2 soft flags (noisy data, not planted fakes).
...
Optional overrides:
    ...
    --strict           tighter thresholds: only flag if score >= 0.55 (targets ~80 honeypots)
"""
...
def main():
    ...
    parser.add_argument("--strict", action="store_true",
                        help="Tighter thresholds targeting ~80 honeypots (default catches ~610)")
